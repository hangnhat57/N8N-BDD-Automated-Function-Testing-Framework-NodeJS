...
export function waitForAttributeToBe(target, attr, value, timeout = DEFAULT_TIMEOUT){
	return browser.wait(
		() => {
			return waitToBeDisplayed(target, timeout)
...
export function waitForAttributeMatch(target, attr, value, timeout = DEFAULT_TIMEOUT) {
	return browser.wait(
		() => {
			return waitToBeDisplayed(target, timeout)
				.then(() => getElementFinder(target).getAttribute(attr))
...
export function waitForElementCountToBe(target, expected, timeout = DEFAULT_TIMEOUT) {
...
export function waitForElementCountToBeGreaterThan(target, expected, timeout = DEFAULT_TIMEOUT) {
	const es = getElementArrayFinder(target);
	return browser.wait(
		() => {
			return es
...
export function waitForElementCountToBeLessThan(target, expected, timeout = DEFAULT_TIMEOUT) {
	const es = getElementArrayFinder(target);
	return browser.wait(
		() => {
			return es
